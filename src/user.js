...
document.onkeydown = e => {
    ...
    switch (e.keyCode) {
        case 37:
            ...
            THE_WORLD.ws.send(JSON.stringify({ method: "CD", dir: LEFT, token: THE_WORLD.player.token }))
            break;
        case 38:
            if (THE_WORLD.player.dir === UP) THE_WORLD.player.force = true;
            else THE_WORLD.player.dir = UP;
            THE_WORLD.ws.send(JSON.stringify({ method: "CD", dir: UP, token: THE_WORLD.player.token }))
            break;
        case 39:
            ...
            THE_WORLD.ws.send(JSON.stringify({ method: "CD", dir: RIGHT, token: THE_WORLD.player.token }))
            break;
        case 40:
            if (THE_WORLD.player.dir === DOWN) THE_WORLD.player.force = true;
            else THE_WORLD.player.dir = DOWN;
            THE_WORLD.ws.send(JSON.stringify({ method: "CD", dir: DOWN, token: THE_WORLD.player.token }))
            break;
        case 190:
            if (window.pause) {
